...
============================
Gera 500 notícias sintéticas para avaliação do experimento de linkage.

Estratégia (Caminho 1 do paper):
...
Resultado:
    - 500 notícias sintéticas com ground truth conhecido (id_processo_licitatorio)
    - Mapeamento JSON de pares para avaliação

Uso:
    python3 gerar_noticias_sinteticas.py                    # executa tudo
    python3 gerar_noticias_sinteticas.py --limit 5          # teste com 5 pares
    python3 gerar_noticias_sinteticas.py --start-from 100   # retomar a partir do par 100
"""
...
# Entradas
NOTICIAS_CSV   = os.path.join(BASE_DIR, "500_noticias_aleatorias.csv")
PROCESSOS_CSV  = os.path.join(BASE_DIR, "500_processos_aleatorios.csv")

# Saídas
OUTPUT_CSV     = os.path.join(BASE_DIR, "500_noticias_sinteticas.csv")
MAPPING_JSON   = os.path.join(BASE_DIR, "mapeamento_pares.json")
LOG_FILE       = os.path.join(BASE_DIR, "geracao_log.txt")
ERROS_JSON     = os.path.join(BASE_DIR, "erros_geracao.json")
STATUS_JSON    = os.path.join(BASE_DIR, "status_execucao.json")
...
# ==============================================================================
def parse_args():
    parser = argparse.ArgumentParser(description="Gera notícias sintéticas para experimento de linkage")
...
def main():
    ...
    print("=" * 65)
    print(" GERAÇÃO DE NOTÍCIAS SINTÉTICAS — Experimento de Linkage")
    print("=" * 65)
    ...
    # 4. Gerar notícias
    log(f"Início da geração: {n_pares} pares (start_from={args.start_from})")
    ...
    for i in pbar:
        ...
        log(f"[{i+1:03d}/{n_pares}] Gerando par: noticia[{i}] ↔ processo {id_processo} ({municipio})")
